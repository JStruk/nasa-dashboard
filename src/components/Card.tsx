...
const Card = ({apod}: CardProps): JSX.Element => {
    return (
        <>
            <article className="flex flex-wrap md:flex-nowrap shadow-lg mx-auto lg:h-80">
                {apod.media_type === 'video'
                    ? <img className="w-full md:w-80 h-auto" src={apod?.thumbnail_url} alt={apod?.title}/>
                    : <img className="w-full md:w-80 h-auto" src={apod?.hdurl} alt={apod?.title}/>
                }
